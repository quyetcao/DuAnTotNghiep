// import React from 'react';

// import css
import '../css/footer.css';

const menu1 = [
    {
        title: 'Tuyến đường',
        subMenu: [
            {
                title: 'Xe đi Buôn Mê Thuột từ Sài Gòn',
                link: 'https://vexere.com/vi-VN/ve-xe-khach-tu-sai-gon-di-buon-ma-thuot-dak-lak-129t21431.html',
            },
            {
                title: 'Xe đi Vũng Tàu từ Sài Gòn',
                link: 'https://vexere.com/vi-VN/ve-xe-khach-tu-sai-gon-di-buon-ma-thuot-dak-lak-129t21431.html',
            },
            {
                title: 'Xe đi Nha Trang từ Sài Gòn',
                link: 'https://vexere.com/vi-VN/ve-xe-khach-tu-sai-gon-di-buon-ma-thuot-dak-lak-129t21431.html',
            },
            {
                title: 'Xe đi Đà Lạt từ Sài Gòn',
                link: 'https://vexere.com/vi-VN/ve-xe-khach-tu-sai-gon-di-buon-ma-thuot-dak-lak-129t21431.html',
            },
            {
                title: 'Xe đi Sapa từ Hà Nội',
                link: 'https://vexere.com/vi-VN/ve-xe-khach-tu-sai-gon-di-buon-ma-thuot-dak-lak-129t21431.html',
            },
            {
                title: 'Xe đi Hải Phòng từ Hà Nội',
                link: 'https://vexere.com/vi-VN/ve-xe-khach-tu-sai-gon-di-buon-ma-thuot-dak-lak-129t21431.html',
            },
        ],
    },
    {
        title: 'Xe Limousine',
        subMenu: [
            {
                title: 'Xe Limousine đi Đà Lạt từ Sài Gòn',
                link: 'https://vexere.com/vi-VN/ve-xe-khach-tu-sai-gon-di-buon-ma-thuot-dak-lak-129t21431.html',
            },
            {
                title: 'Xe Limousine đi Vũng Tàu từ Sài Gòn',
                link: 'https://vexere.com/vi-VN/ve-xe-khach-tu-sai-gon-di-buon-ma-thuot-dak-lak-129t21431.html',
            },
            {
                title: 'Xe Limousine đi Nha Trang từ Sài Gòn',
                link: 'https://vexere.com/vi-VN/ve-xe-khach-tu-sai-gon-di-buon-ma-thuot-dak-lak-129t21431.html',
            },
            {
                title: 'Xe Limousine đi Hải Phòng từ Hà Nội',
                link: 'https://vexere.com/vi-VN/ve-xe-khach-tu-sai-gon-di-buon-ma-thuot-dak-lak-129t21431.html',
            },
            {
                title: 'Xe Limousine đi Hạ Long từ Hà Nội',
                link: 'https://vexere.com/vi-VN/ve-xe-khach-tu-sai-gon-di-buon-ma-thuot-dak-lak-129t21431.html',
            },
            {
                title: 'Xe Limousine đi Sapa Từ Hà Nội',
                link: 'https://vexere.com/vi-VN/ve-xe-khach-tu-sai-gon-di-buon-ma-thuot-dak-lak-129t21431.html',
            },
            {
                title: 'Xe Limousine đi Quảng Ninh từ Hà Nội',
                link: 'https://vexere.com/vi-VN/ve-xe-khach-tu-sai-gon-di-buon-ma-thuot-dak-lak-129t21431.html',
            },
        ],
    },
    {
        title: 'Tin tức',
        subMenu: [
            {
                title: 'Xe giường nằm Limousine – đỉnh cao mới của ngành xe khách',
                link: 'https://vexere.com/vi-VN/ve-xe-khach-tu-sai-gon-di-buon-ma-thuot-dak-lak-129t21431.html',
            },
            {
                title: 'Xe limousine đi Vũng Tàu: Tổng hợp top 6 xe chất lượng cao',
                link: 'https://vexere.com/vi-VN/ve-xe-khach-tu-sai-gon-di-buon-ma-thuot-dak-lak-129t21431.html',
            },
            {
                title: 'Review xe limousine đi Đà Lạt: những câu hỏi thường gặp',
                link: 'https://vexere.com/vi-VN/ve-xe-khach-tu-sai-gon-di-buon-ma-thuot-dak-lak-129t21431.html',
            },
            {
                title: 'Xe limousine đi Sapa: Tổng hợp top các hãng xe chất lượng cao',
                link: 'https://vexere.com/vi-VN/ve-xe-khach-tu-sai-gon-di-buon-ma-thuot-dak-lak-129t21431.html',
            },
        ],
    },
];

const menu2 = [
    {
        title: 'Bến xe',
        subMenu: [
            {
                title: 'Bến xe Miền Đông',
            },
            {
                title: 'Bến xe Trung tâm Đà Nẵng',
            },
            {
                title: 'Bến xe Gia Lâm',
            },
            {
                title: 'Bến xe Mỹ Đình',
            },
            {
                title: 'Bến xe An Sương',
            },
            {
                title: 'Bến xe Nước Ngầm',
            },
            {
                title: 'Bến xe Miền Tây',
            },
        ],
    },
    
];

const menu3 = [
    {
        title: 'Hỗ trợ',
        subMenu: [
            {
                title: 'Hướng dẫn thanh toán',
            },
            {
                title: 'Quy chế Vexere.com',
            },
            {
                title: 'Chính sách bảo mật thông tin',
            },
            {
                title: 'Chính sách bảo mật thanh toán',
            },
            {
                title: 'Chính sách và quy trình giải quyết tranh chấp, khiếu nại',
            },
            {
                title: 'Câu hỏi thường gặp',
            },
        ],
    },
];


export default function Footer() {
    return (
        <div className='footer'>
            <div className='box2-footer'>
                <div className='footer-top'>
                    <div className='footer-content'>
                        <div className='content-top'>
                            {menu1.map((item, index) => (
                                <div className='content-box' key={index}>
                                    <p className='content-title'>{item.title}</p>
                                    <div className='content-list__car'>
                                        <ul className='footer-list'>
                                            {item.subMenu.map((itemSubMenu, i) => (
                                                <li className='footer-item' key={i}>
                                                    <a href={itemSubMenu.link} className='footer-link link-text'>
                                                        {itemSubMenu.title}
                                                    </a>
                                                </li>
                                            ))}
                                        </ul>
                                    </div>
                                </div>
                            ))}
                        </div>
                        <div className='content-bottom'>
                            {menu2.map((item, index) => (
                                <div className='bus-station' key={index}>
                                    <p className='content-title'>{item.title}</p>
                                    <div className='content-list__car'>
                                        <ul className='footer-list'>
                                            {item.subMenu.map((itemSubMenu, i) => (
                                                <li className='footer-item' key={i}>
                                                    <a href={itemSubMenu.link} className='footer-link link-text'>
                                                        {itemSubMenu.title}
                                                    </a>
                                                </li>
                                            ))}
                                        </ul>
                                    </div>
                                </div>
                            ))}
                            
                            
                            <div className='garage'>
                                <p className='content-title'>Nhà xe</p>
                                <div className='hhh'>
                                    <div className='nha-xe'>
                                        <a
                                            href='https://vexere.com/vi-VN/xe-lien-hung'
                                            target='_blank'
                                            rel='noopener noreferrer'
                                            className='footer-link link-text'
                                        >
                                            Xe Liên Hưng
                                        </a>
                                    </div>
                                    <div className='nha-xe'>
                                        <a
                                            href='https://vexere.com/vi-VN/xe-phong-phu'
                                            target='_blank'
                                            rel='noopener noreferrer'
                                            className='footer-link link-text'
                                        >
                                            Xe Phong Phú
                                        </a>
                                    </div>
                                    <div className='nha-xe'>
                                        <a
                                            href='https://vexere.com/vi-VN/xe-tien-oanh'
                                            target='_blank'
                                            rel='noopener noreferrer'
                                            className='footer-link link-text'
                                        >
                                            Xe Tiến Oanh
                                        </a>
                                    </div>
                                    <div className='nha-xe'>
                                        <a
                                            href='https://vexere.com/vi-VN/xe-tuan-hung'
                                            target='_blank'
                                            rel='noopener noreferrer'
                                            className='footer-link link-text'
                                        >
                                            Xe Tuấn Hưng
                                        </a>
                                    </div>
                                </div>
                                <div className='hhh'>
                                    <div className='nha-xe'>
                                        <a
                                            href='https://vexere.com/vi-VN/xe-khanh-phong'
                                            target='_blank'
                                            rel='noopener noreferrer'
                                            className='footer-link link-text'
                                        >
                                            Xe Khanh Phong
                                        </a>
                                    </div>
                                    <div className='nha-xe'>
                                        <a
                                            href='https://vexere.com/vi-VN/xe-dong-phuoc'
                                            target='_blank'
                                            rel='noopener noreferrer'
                                            className='footer-link link-text'
                                        >
                                            Xe Đồng Phước
                                        </a>
                                    </div>
                                    <div className='nha-xe'>
                                        <a
                                            href='https://vexere.com/vi-VN/xe-thuan-tien'
                                            target='_blank'
                                            rel='noopener noreferrer'
                                            className='footer-link link-text'
                                        >
                                            Xe Thuận Tiến
                                        </a>
                                    </div>
                                    <div className='nha-xe'>
                                        <a
                                            href='https://vexere.com/vi-VN/xe-long-van-limousine'
                                            target='_blank'
                                            rel='noopener noreferrer'
                                            className='footer-link link-text'
                                        >
                                            Xe Long Vân Limousine
                                        </a>
                                    </div>
                                </div>
                                <div className='hhh'>
                                    <div className='nha-xe'>
                                        <a
                                            href='https://vexere.com/vi-VN/xe-cuc-tung'
                                            target='_blank'
                                            rel='noopener noreferrer'
                                            className='footer-link link-text'
                                        >
                                            Xe Cúc Tùng
                                        </a>
                                    </div>
                                    <div className='nha-xe'>
                                        <a
                                            href='https://vexere.com/vi-VN/xe-an-phu'
                                            target='_blank'
                                            rel='noopener noreferrer'
                                            className='footer-link link-text'
                                        >
                                            Xe An Phú Buslines
                                        </a>
                                    </div>
                                    <div className='nha-xe'>
                                        <a
                                            href='https://vexere.com/vi-VN/xe-bang-phan'
                                            target='_blank'
                                            rel='noopener noreferrer'
                                            className='footer-link link-text'
                                        >
                                            Xe Bằng Phấn
                                        </a>
                                    </div>
                                    <div className='nha-xe'>
                                        <a
                                            href='https://vexere.com/vi-VN/xe-an-phu-quy'
                                            target='_blank'
                                            rel='noopener noreferrer'
                                            className='footer-link link-text'
                                        >
                                            Xe An Phú Quý
                                        </a>
                                    </div>
                                </div>
                                <div className='hhh'>
                                    <div className='nha-xe'>
                                        <a
                                            href='https://vexere.com/vi-VN/xe-36-limousine'
                                            target='_blank'
                                            rel='noopener noreferrer'
                                            className='footer-link link-text'
                                        >
                                            Xe 36 Limousine
                                        </a>
                                    </div>
                                    <div className='nha-xe'>
                                        <a
                                            href='https://vexere.com/vi-VN/xe-phuc-xuyen'
                                            target='_blank'
                                            rel='noopener noreferrer'
                                            className='footer-link link-text'
                                        >
                                            Xe Phúc Xuyên
                                        </a>
                                    </div>
                                    <div className='nha-xe'>
                                        <a
                                            href='https://vexere.com/vi-VN/xe-ha-lan'
                                            target='_blank'
                                            rel='noopener noreferrer'
                                            className='footer-link link-text'
                                        >
                                            Xe Hà Lan
                                        </a>
                                    </div>
                                    <div className='nha-xe'>
                                        <a
                                            href='https://vexere.com/vi-VN/xe-tan-kim-chi'
                                            target='_blank'
                                            rel='noopener noreferrer'
                                            className='footer-link link-text'
                                        >
                                            Xe Tân Kim Chi
                                        </a>
                                    </div>
                                </div>
                                <div className='hhh'>
                                    <div className='nha-xe'>
                                        <a
                                            href='https://vexere.com/vi-VN/xe-minh-quoc'
                                            target='_blank'
                                            rel='noopener noreferrer'
                                            className='footer-link link-text'
                                        >
                                            Xe Minh Quốc
                                        </a>
                                    </div>
                                    <div className='nha-xe'>
                                        <a
                                            href='https://vexere.com/vi-VN/xe-hai-au'
                                            target='_blank'
                                            rel='noopener noreferrer'
                                            className='footer-link link-text'
                                        >
                                            Xe Hải Âu
                                        </a>
                                    </div>
                                    <div className='nha-xe'>
                                        <a
                                            href='https://vexere.com/vi-VN/xe-g8-sapa-open-tour'
                                            target='_blank'
                                            rel='noopener noreferrer'
                                            className='footer-link link-text'
                                        >
                                            Xe G8 Open Tour
                                        </a>
                                    </div>
                                    <div className='nha-xe'>
                                        <a
                                            href='https://vexere.com/vi-VN/xe-tra-lan-vien'
                                            target='_blank'
                                            rel='noopener noreferrer'
                                            className='footer-link link-text'
                                        >
                                            Xe Trà Lan Viên
                                        </a>
                                    </div>
                                </div>
                                <div className='hhh'>
                                    <div className='nha-xe'>
                                        <a
                                            href='https://vexere.com/vi-VN/xe-vie-limousine'
                                            target='_blank'
                                            rel='noopener noreferrer'
                                            className='footer-link link-text'
                                        >
                                            Xe Vie Limousine
                                        </a>
                                    </div>
                                    <div className='nha-xe'>
                                        <a
                                            href='https://vexere.com/vi-VN/xe-hao'
                                            target='_blank'
                                            rel='noopener noreferrer'
                                            className='footer-link link-text'
                                        >
                                            Xe Hảo
                                        </a>
                                    </div>
                                    <div className='nha-xe'>
                                        <a
                                            href='https://vexere.com/vi-VN/xe-dien-linh-limousine'
                                            target='_blank'
                                            rel='noopener noreferrer'
                                            className='footer-link link-text'
                                        >
                                            Xe Điền Linh
                                        </a>
                                    </div>
                                    <div className='nha-xe'>
                                        <a
                                            href='https://vexere.com/vi-VN/xe-nam-cuong-limousine'
                                            target='_blank'
                                            rel='noopener noreferrer'
                                            className='footer-link link-text'
                                        >
                                            Xe Nam Cường Limousine
                                        </a>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div className='footer-bottom'>
                    <div className='footer-content'>
                        <div className='footer-left'>
                            <div className='footer-col'>
                            {menu3.map((item, index) => (
                                <div className='content-box' key={index}>
                                    <p className='content-title'>{item.title}</p>
                                    <div className='content-list__car'>
                                        <ul className='footer-list'>
                                            {item.subMenu.map((itemSubMenu, i) => (
                                                <li className='footer-item' key={i}>
                                                    <a href={itemSubMenu.link} className='footer-link link-text'>
                                                        {itemSubMenu.title}
                                                    </a>
                                                </li>
                                            ))}
                                        </ul>
                                    </div>
                                </div>
                            ))}
                        </div>
                            <div className='footer-col flex-col'>
                                <div className='footer-intro'>
                                    <p className='content-title'>Về chúng tôi</p>
                                    <ul className='footer-list'>
                                        <li className='footer-item'>
                                            <a href='' className='footer-link link-text'>
                                                Phần mềm đại lý
                                            </a>
                                        </li>
                                        <li className='footer-item'>
                                            <a href='' className='footer-link link-text'>
                                                Giới Thiệu Vexere.com
                                            </a>
                                        </li>
                                        <li className='footer-item'>
                                            <a href='' className='footer-link link-text'>
                                                Tuyển dụng
                                            </a>
                                        </li>
                                        <li className='footer-item'>
                                            <a href='' className='footer-link link-text'>
                                                Tin tức
                                            </a>
                                        </li>
                                        <li className='footer-item'>
                                            <a href='' className='footer-link link-text'>
                                                Liên hệ
                                            </a>
                                        </li>
                                    </ul>
                                    <div className='footer-channel'>
                                        <a
                                            href='https://www.facebook.com/VexereOfficial'
                                            target='_blank'
                                            rel='noreferrer'
                                        >
                                            <img
                                                src='https://229a2c9fe669f7b.cmccloud.com.vn/svgIcon/footer-social-facebook.svg'
                                                alt='footer-social-facebook'
                                            />
                                        </a>
                                        <a
                                            href='https://www.youtube.com/channel/UCpAkiqkvMD4TtksSiCZTM0g'
                                            target='_blank'
                                            rel='noreferrer'
                                        >
                                            <img
                                                src='https://229a2c9fe669f7b.cmccloud.com.vn/svgIcon/footer-social-youtube.svg'
                                                alt='footer-social-youtube'
                                            />
                                        </a>
                                        <a
                                            href='https://www.tiktok.com/@vexere.official'
                                            target='_blank'
                                            rel='noreferrer'
                                        >
                                            <img
                                                src='https://229a2c9fe669f7b.cmccloud.com.vn/svgIcon/footer-social-tiktok.svg'
                                                alt='footer-social-tiktok'
                                            />
                                        </a>
                                    </div>
                                </div>
                                <div className='footer-certification'>
                                    <p className='content-title'>Chứng nhận</p>
                                    <div className='certification-container'>
                                        <img
                                            className='passenger-icon ls-is-cached lazyloaded'
                                            data-src='https://229a2c9fe669f7b.cmccloud.com.vn/images/Desktop_Cert_1.png'
                                            src='https://229a2c9fe669f7b.cmccloud.com.vn/images/Desktop_Cert_1.png'
                                            alt='passenger-icon'
                                            width='200'
                                            height='auto'
                                        />
                                        <div className='logo-certs'>
                                            <a href='http://online.gov.vn/Home/AppDetails/462'>
                                                <img
                                                    className='passenger-icon ls-is-cached lazyloaded'
                                                    data-src='https://229a2c9fe669f7b.cmccloud.com.vn/images/Desktop_Cert_2.png'
                                                    src='https://229a2c9fe669f7b.cmccloud.com.vn/images/Desktop_Cert_2.png'
                                                    alt='passenger-icon'
                                                    width='100'
                                                    height='auto'
                                                />
                                            </a>
                                            <a href='http://online.gov.vn/Home/WebDetails/70277'>
                                                <img
                                                    className='passenger-icon ls-is-cached lazyloaded'
                                                    data-src='https://229a2c9fe669f7b.cmccloud.com.vn/images/Desktop_Cert_2.png'
                                                    src='https://229a2c9fe669f7b.cmccloud.com.vn/images/Desktop_Cert_2.png'
                                                    alt='passenger-icon'
                                                    width='100'
                                                    height='auto'
                                                />
                                            </a>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div className='footer-col'>
                                <p className='content-title'>Đối tác thanh toán</p>
                                <img
                                    className='passenger-icon ls-is-cached lazyloaded'
                                    data-src='https://229a2c9fe669f7b.cmccloud.com.vn/images/payment_partner_desktop.png'
                                    src='https://229a2c9fe669f7b.cmccloud.com.vn/images/payment_partner_desktop.png'
                                    alt='passenger-icon'
                                    width='202'
                                    height='344'
                                />
                            </div>
                            <div className='footer-col'>
                                <p className='content-title'>Tải ứng dụng Vexere</p>
                                <div className='download-app__qr'>
                                    <li className='download-app__qr-item'>
                                        <img
                                            className='qr'
                                            src='https://229a2c9fe669f7b.cmccloud.com.vn/images/landingpagetet2023/download_app_qr.png'
                                            alt='landingpagetet2023/download_app_qr'
                                            height='164'
                                        />
                                    </li>
                                    <li className='download-app__qr-item'>
                                        <a
                                            className='download-app-store-button'
                                            href='https://itunes.apple.com/vn/app/vexere/id1183279479'
                                            target='_blank'
                                            rel='noreferrer'
                                        >
                                            <img
                                                className='lazyloaded'
                                                src='https://229a2c9fe669f7b.cmccloud.com.vn/images/download-app-store.png'
                                                alt='download-app-store-img'
                                                width='164'
                                                height='48'
                                            />
                                        </a>
                                    </li>
                                    <li className='download-app__qr-item'>
                                        <a
                                            className='download-gg-play-button'
                                            href='https://play.google.com/store/apps/details?id=com.vexere.vexere'
                                            target='_blank'
                                            rel='noreferrer'
                                        >
                                            <img
                                                className='lazyloaded'
                                                src='https://229a2c9fe669f7b.cmccloud.com.vn/images/download-gg-play.png'
                                                alt='download-gg-play-img'
                                                width='164'
                                                height='48'
                                            />
                                        </a>
                                    </li>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div className='footer-end'>
                <div className='footer-end-center'>
                    <p className='ft-end__heading'>Công ty TNHH Thương Mại Dịch Vụ Vexere</p>
                    <div className='link-description'>
                        <p className='link-title'>
                            Địa chỉ đăng ký kinh doanh: 8C Chữ Đồng Tử, Phường 7, Quận Tân Bình, Thành Phố Hồ Chí Minh,
                            Việt Nam
                        </p>
                    </div>
                    <div className='footer-link-text'>
                        <span>
                            Địa chỉ:
                            <a
                                href='https://www.google.com/maps/place/CirCO+Coworking+Space/@10.7603287,106.6965187,17z/data=!3m1!4b1!4m6!3m5!1s0x31752f13392ff913:0x1802e764b3059cae!8m2!3d10.7603234!4d106.6990936!16s%2Fg%2F11c1xf__g9?hl=en-US&amp;entry=ttu'
                                rel='nofollow noopener noreferrer'
                                title='Xem bản đồ'
                                target='_blank'
                                className='no-title'
                            >
                                <span itemProp='address' itemType='http://schema.org/PostalAddress'>
                                    <span itemProp='streetAddress' className='end-title'>
                                        Lầu 2, tòa nhà H3 Circo Hoàng Diệu, 384 Hoàng Diệu, Phường 6, Quận 4, Tp. Hồ Chí
                                        Minh, Việt Nam
                                    </span>
                                </span>
                            </a>
                            <br />
                            <a
                                href='https://www.google.com/search?kgmid=/g/11pphhw0d_&amp;hl=vi-VN&amp;q=V%C4%83n+ph%C3%B2ng+%C4%91%E1%BA%A1i+di%E1%BB%87n+t%E1%BA%A1i+H%C3%A0+N%E1%BB%99i+-+C%C3%B4ng+ty+TNHH+Th%C6%B0%C6%A1ng+m%E1%BA%A1i+d%E1%BB%8Bch+v%E1%BB%A5+VEXERE&amp;kgs=f1aa79a4fe01a36a&amp;shndl=30&amp;shem=bsc&amp;source=sh/x/loc/osrp/m5/1'
                                rel='nofollow noopener noreferrer'
                                title='Xem bản đồ'
                                target='_blank'
                                className='no-title'
                            >
                                <span itemProp='address' itemType='http://schema.org/PostalAddress'>
                                    <span itemProp='streetAddress' className='end-title'>
                                        Tầng 3, Toà nhà 101 Láng Hạ, Phường Láng Hạ, Quận Đống Đa, Hà Nội, Việt Nam
                                    </span>
                                </span>
                            </a>
                            <br />
                            <span className='end-title'>
                                Giấy chứng nhận ĐKKD số 0315133726 do Sở KH và ĐT TP. Hồ Chí Minh cấp lần đầu ngày
                                27/6/2018
                            </span>
                            <div className=''>Bản quyền © 2024 thuộc về Vexere.com</div>
                        </span>
                    </div>
                </div>
            </div>
        </div>
    );
}
